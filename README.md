...
| `uni2_interactions.py` | Screens every variable **pair** for joint prediction of survival (within-cancer) and tumor type — dim×dim and feature×dim. Vectorized all-pairs interaction–outcome correlation (matmul), a family-wise **permutation null**, and a **cross-validated incremental test** (does i×j beat i+j on held-out C-index/AUC?). Centers+standardizes per fold. Outputs `results_v2/dim_omics/interactions_*.csv` (gitignored) + `figures/interactions_*_null.png`. Finding so far: tumor-type interactions screen strongly but add ~nothing over main effects; survival interactions are weak, a small set passing both the null and the held-out test. Dims are plain numbers; feature codes are hashed (no names). |
